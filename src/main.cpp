...
#if defined(NDEBUG)
# error "Gobblecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8ff3424dbd004c954806723fe7f9f2124087144f9e40e6729d88541a6975c1fc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gobblecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gobblecoin Signed Message:\n";
...
{
    // Gobblecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Gobblecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Gobblecoin: 1.5 days
static const int64 nTargetSpacing = 1.5 * 60; // Gobblecoin: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gobblecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gobblecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9bdceec87c82b5a58995c8f5412c9566f819e88a71b13b47724262116db2efdf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CBC 11/13/2021 Elon Musk ends the week selling another $1.2 billion of Tesla shares";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042d0dc75fd0ae2711f4cccbd2eda4f63baeed19a7c7de45003241081871a1d1b2554418500635a7445976f05365c91a428e57b0cb2135f38a65e1f0f80a7aa34a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1636856635;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087012017;

        if (fTestNet)
        {
            block.nTime    = 1636856626;
            block.nNonce   = 385808588;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xbcd228fb374a8dafeb1117f9d57d6e94b28092a370d477a9a77065f45b585024"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa1, 0xb2, 0xc5, 0xd3 }; // Gobblecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GobblecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GobblecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GobblecoinMiner(CWallet *pwallet)
{
    printf("GobblecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gobblecoin-miner");
    ...
    try { loop {
        ...
        printf("Running GobblecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GobblecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GobblecoinMiner, pwallet));
}
